...
class SpritesEnv(gym.Env):
    ...
    def __init__(self, repeats=5, max_episode_length=50, images_archive=None, difficulty="hard"):
        """
        Constructor of a dSprite environment (compatible with OpenAI gym environment)
        :param repeats: the number of time an action is repeated within each action perception cycle
        :param max_episode_length: the maximum number of action perception cycle before a trial is a failure
        :param images_archive: the path to the file containing the dSprites dataset
        :param difficulty: the difficulty of the environment either "easy" or "hard"
        """
        ...
        if images_archive is None:
            images_archive = f"{os.environ['DATA_DIRECTORY']}/dsprites.npz"
        self.images, self.s_sizes, self.s_dim, self.s_bases = DataSet.get(images_archive)
